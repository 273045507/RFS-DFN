...
class SensorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(sensor_file):
            ...
            sensor_data = sensor_data_df.iloc[:, :7].values.astype(float)  # 假设取前两列为传感器数据
        else:
...
# model.load_state_dict(torch.load("best_model10.pth"))
...
optimizer_transformer = torch.optim.Adam(
    list(model.transformer_encoder.parameters()) + list(model.regressor.parameters()),
    lr=1e-4
)
...
# 继续训练
for epoch in range(10):  # 从101轮开始到200轮
    model.train()
    running_loss = 0.0
    correct_train = 0
    total_train = 0

    for features, wear_value in train_loader:
        features, wear_value = features.to(device), wear_value.to(device)
        ...
        # 计算训练准确率
        _, predicted_classes = torch.max(wear_pred, dim=1)
        correct_train += (predicted_classes == wear_value).sum().item()
        total_train += wear_value.size(0)
    ...
    if val_accuracy > best_val_accuracy:
        ...
        print("保存新的最佳模型：best_model11.pth")
    else:
