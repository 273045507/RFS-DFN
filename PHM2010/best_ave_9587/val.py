...
class FeatureFusionTransformer(nn.Module):
    def __init__(self, input_dim, reduced_dim, num_layers=2, nhead=4):
        ...
        # Transformer编码器
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=reduced_dim, nhead=nhead,
                                                        batch_first=True)
    ...
    def forward(self, features):
        ...
        output = transformer_output.mean(dim=1)  # 改为 dim=1 来保持 batch 维度
...
# 实例化数据集
train_dataset = SensorDataset(path, train_set_ids)
...
def calculate_accuracy(predictions, targets):
    ...
    correct = (predicted_classes == targets).sum().item()  # 计算正确预测的数量
...
def validate_and_save(model, dataloader, criterion, device, output_file="validation_results.csv"):
    """
    验证模型，计算验证集的损失、准确率和召回率，并将实际和预测标签保存到CSV文件中。

    :param model: 待验证的模型
    :param dataloader: 验证集的 DataLoader
    :param criterion: 损失函数
    :param device: 设备 (CPU/GPU)
    :param output_file: 输出保存的CSV文件名
    :return: (验证损失, 验证准确率, 验证召回率)
    """
    model.eval()
    running_loss = 0.0
    all_labels = []
    ...
    with open(output_file, mode='w', newline='') as csv_file:
        ...
        writer.writerow(["Actual Label", "Predicted Label"])  # 表头
        writer.writerows(zip(all_labels, all_predictions))

    print(f"Validation Loss: {val_loss:.4f}, Accuracy: {val_accuracy:.4f}, Recall: {val_recall:.4f}")
    print(f"Validation results saved to {output_file}")

    return val_loss, val_accuracy, val_recall

val_loss, val_accuracy, val_recall = validate_and_save(
    model, val_loader, criterion, device, output_file="validation_results.csv"
)
